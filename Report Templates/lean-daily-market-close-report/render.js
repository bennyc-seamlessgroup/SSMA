...
function tr(value) { return window.ReportI18n?.translate(reportLanguage, value) ?? String(value ?? ''); }
...
function formatReportDate(value) {
  ...
  const datePart = isoDatePart(value);
  const parsed = datePart ? new Date(`${datePart}T12:00:00Z`) : new Date(String(value ?? ''));
  if (!Number.isFinite(parsed.getTime())) return String(value ?? '');
  return new Intl.DateTimeFormat(reportLocale, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' }).format(parsed);
}
...
function formatGeneratedAt(data) {
  ...
  return new Intl.DateTimeFormat(reportLocale, { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'America/New_York' }).format(parsed);
}
