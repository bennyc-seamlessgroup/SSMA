...
test('uses suggestion records to calculate the next reported total without publishing them to Ownership', () => {
  const holders = currentHoldersFromRecords([
    { id: 'one', holderName: 'Suggested Capital', category: 'Strategic Investor', shares: 1_000, action: 'add', showInOwnership: false, showAsSuggestion: true, status: 'pending' },
    { id: 'ignored', holderName: 'Hidden Holder', shares: 500, action: 'add', showInOwnership: false, showAsSuggestion: false, status: 'pending' },
  ]);
  assert.equal(holders.length, 1);
  assert.equal(holders[0].holderName, 'Suggested Capital');
  assert.equal(holders[0].priorTotalShares, 1_000);
});
